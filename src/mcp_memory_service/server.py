    ...
    def register_handlers(self):
        ...
        async def handle_list_tools() -> List[types.Tool]:
            ...
            try:
                ...
                # ========== PHASE 2-4: Memory Supercharge Tools ==========
                supercharge_tools = [
                    types.Tool(
                        name="hybrid_retrieve_memory",
                        description="""Hybrid search combining semantic and keyword matching.

                        Uses Reciprocal Rank Fusion (RRF) to merge results from:
                        - Semantic search via Vectorize (understands meaning)
                        - Keyword search via FTS5/LIKE (finds exact terms)

                        Best for queries that need both semantic understanding and exact matches.

                        Example:
                        {
                            "query": "SSH port 65002 error",
                            "n_results": 10,
                            "semantic_weight": 0.6,
                            "keyword_weight": 0.4
                        }""",
                        inputSchema={
                            ...
                            "properties": {
                                ...
                                "semantic_weight": {
                                    "type": "number",
                                    "description": "Weight for semantic search 0-1 (default: 0.6)",
                                    "default": 0.6
                                },
                                "keyword_weight": {
                                    "type": "number",
                                    "description": "Weight for keyword search 0-1 (default: 0.4)",
                                    "default": 0.4
                                },
                                "project": {
                                    "type": "string",
                    ...
                    types.Tool(
                        name="smart_retrieve_memory",
                        description="""Smart memory search with automatic query understanding.

                        Automatically detects from your query:
                        - Project names (freel, mind-space, etc.)
                        - Memory types (task, error, decision)
                        - Time expressions (last week, yesterday, 3 days ago)
                        - Status (completed, pending, blocked)
                        - Abbreviations (wp→wordpress, cf→cloudflare)

                        Example:
                        {
                            "query": "freel elementor issue last week"
                        }

                        Will automatically apply: project=freel, type=error, time=7 days""",
                        inputSchema={
                            "type": "object",
                            "properties": {
                                "query": {
                    ...
                    types.Tool(
                        ...
                        description="""Find semantically similar memories that may be duplicates.

                        Scans recent memories and finds pairs with high similarity scores.
                        Returns recommendations: keep_both, review_and_merge, delete_first, delete_second.
                        ...
                        inputSchema={
                            ...
                            "properties": {
                                "similarity_threshold": {
                                    "type": "number",
                                    "description": "Minimum similarity to consider duplicate 0-1 (default: 0.90)",
                                    "default": 0.90
                                },
                                "max_results": {
                                    "type": "integer",
                    ...
                    types.Tool(
                        name="consolidate_memories",
                        description="""Merge multiple similar memories into one.

                        Keeps one memory and deletes the rest. Optionally merges tags.
                        ...
                        inputSchema={
                            ...
                            "properties": {
                                "hashes": {
                                    "type": "array",
                                    "items": {"type": "string"},
                                    "description": "List of memory hashes to consolidate"
                                },
                                "keep_hash": {
                                    "type": "string",
                                ...
                                "merge_tags": {
                                    "type": "boolean",
                                    "description": "Merge tags from deleted memories (default: true)",
                                    "default": True
                                }
                            },
                            "required": ["hashes", "keep_hash"]
                        }
                    ),
                    types.Tool(
                        name="cleanup_duplicates_auto",
                        description="""Automatically clean up old near-duplicates.

                        Finds highly similar (>=98%) memories older than specified days
                        and optionally deletes them. Use dry_run=true to preview.
                        ...
                        inputSchema={
                            ...
                            "properties": {
                                "max_age_days": {
                                    "type": "integer",
                                    "description": "Only process memories older than this (default: 90)",
                                    "default": 90
                                },
                                "similarity_threshold": {
                                    "type": "number",
                                    "description": "Only auto-delete if similarity >= this (default: 0.98)",
                                    "default": 0.98
                                },
                                "dry_run": {
                                    "type": "boolean",
                                    "description": "If true, only report without deleting (default: true)",
                                    "default": True
                                }
                            }
                        }
                    ...
                    types.Tool(
                        ...
                        description="""Find tasks that are in-progress for too long.

                        Helps identify forgotten or stuck tasks that need attention.
                        ...
                        inputSchema={
                            ...
                            "properties": {
                                "days": {
                                    "type": "integer",
                                    "description": "Consider stale if in-progress longer than N days (default: 7)",
                                    "default": 7
                                },
                                "project": {
                                    "type": "string",
                    ...
                    types.Tool(
                        ...
                        description="""Load full context for a project - perfect for starting a new session.

                        Returns pending tasks, in-progress tasks, recent sessions, decisions,
                        errors, config, and client chats for the specified project.
                        ...
                        inputSchema={
                            ...
                            "properties": {
                                "project": {
                                    "type": "string",
                                    "description": "Project name (e.g., 'freel', 'mind-space')"
                                },
                                "include_tasks": {
                                    "type": "boolean",
                                    "description": "Include pending/in-progress tasks (default: true)",
                                    "default": True
                                },
                                "include_recent": {
                                    "type": "boolean",
                                    "description": "Include recent sessions/decisions/errors (default: true)",
                                    "default": True
                                },
                                "include_config": {
                                    "type": "boolean",
                                    "description": "Include config and setup memories (default: true)",
                                    "default": True
                                },
                                "days": {
                                    "type": "integer",
                                    "description": "Look back this many days for recent items (default: 14)",
                                    "default": 14
                                }
                            },
                            "required": ["project"]
                    ...
                    types.Tool(
                        ...
                        Call this before context overflow or session end to preserve progress.

                        Example:
                        {
                            "project": "freel",
                            "current_task": "Implementing user auth",
                            "progress": "Added login form, connected to API",
                            "next_steps": ["Add validation", "Add error handling", "Write tests"]
                        }""",
                        inputSchema={
                            "type": "object",
                            "properties": {
                                ...
                                "next_steps": {
                                    "type": "array",
                                    "items": {"type": "string"},
                                    "description": "List of next steps to do"
                                },
                                "blockers": {
                                    "type": "array",
                                    "items": {"type": "string"},
                            ...
                            "required": ["project", "current_task", "progress", "next_steps"]
                        }
                    ),
                    types.Tool(
                        name="get_interrupted_sessions",
                        description="""Find sessions that were saved but not completed.

                        Use at session start to resume interrupted work.
                    ...
                    types.Tool(
                        ...
                        description="""Get analytics about stored memories.

                        Returns counts by type, project, status, and time distribution.
                        ...
                        inputSchema={
                            ...
                            "properties": {
                                ...
                                "days": {
                                    "type": "integer",
                                    "description": "Analyze last N days (default: 30)",
                                    "default": 30
                                }
                            }
                        }
                    ...
                    types.Tool(
                        ...
                        description="""Safely clean up old memories.

                        SAFE BY DEFAULT: dry_run=true shows what would be deleted without deleting.
                        Set dry_run=false to actually delete.

                        Example:
                        {
                            "older_than_days": 90,
                            "memory_types": ["session-log"],
                            "dry_run": true
                        }""",
                        inputSchema={
                            "type": "object",
                            "properties": {
                                "older_than_days": {
                                    "type": "integer",
                                    "description": "Delete memories older than this (default: 90)",
                                    "default": 90
                                },
                                "memory_types": {
                                    "type": "array",
                                    "items": {"type": "string"},
                                    "description": "Optional: only delete these types (e.g., ['session-log', 'session'])"
                                },
                                "exclude_tags": {
                                    "type": "array",
                                    "items": {"type": "string"},
                                    "description": "Optional: never delete memories with these tags (e.g., ['important', 'keep'])"
                                },
                                "dry_run": {
                                    "type": "boolean",
                                    "description": "If true, only preview without deleting (default: true)",
                                    "default": True
                                }
                            }
                        }
                ...
                tools.extend(supercharge_tools)
                logger.info(f"Added {len(supercharge_tools)} Memory Supercharge tools")
        ...
        async def handle_call_tool(name: str, arguments: dict | None) -> List[types.TextContent]:
            ...
            try:
                ...
                # ========== PHASE 2-4: Memory Supercharge Handlers ==========
                elif name == "hybrid_retrieve_memory":
                    logger.info("Calling handle_hybrid_retrieve_memory")
                    return await self.handle_hybrid_retrieve_memory(arguments)
                elif name == "smart_retrieve_memory":
                    logger.info("Calling handle_smart_retrieve_memory")
                    return await self.handle_smart_retrieve_memory(arguments)
                elif name == "find_duplicate_memories":
                    logger.info("Calling handle_find_duplicate_memories")
                    return await self.handle_find_duplicate_memories(arguments)
                elif name == "consolidate_memories":
                    logger.info("Calling handle_consolidate_memories")
                    return await self.handle_consolidate_memories(arguments)
                elif name == "cleanup_duplicates_auto":
                    logger.info("Calling handle_cleanup_duplicates_auto")
                    return await self.handle_cleanup_duplicates_auto(arguments)
                # ========== WORKFLOW IMPROVEMENT HANDLERS ==========
                elif name == "find_stale_tasks":
                    logger.info("Calling handle_find_stale_tasks")
                    return await self.handle_find_stale_tasks(arguments)
                elif name == "load_project_context":
                    logger.info("Calling handle_load_project_context")
                    return await self.handle_load_project_context(arguments)
                elif name == "save_session_state":
                    logger.info("Calling handle_save_session_state")
                    return await self.handle_save_session_state(arguments)
                elif name == "get_interrupted_sessions":
                    logger.info("Calling handle_get_interrupted_sessions")
                    return await self.handle_get_interrupted_sessions(arguments)
                elif name == "get_memory_analytics":
                    logger.info("Calling handle_get_memory_analytics")
                    return await self.handle_get_memory_analytics(arguments)
                elif name == "cleanup_old_memories":
                    logger.info("Calling handle_cleanup_old_memories")
                    return await self.handle_cleanup_old_memories(arguments)
                else:
    ...
    async def handle_delete_memory(self, arguments: dict) -> List[types.TextContent]:
        ...
        try:
            ...
            if result.get("success"):
                return [types.TextContent(type="text", text=f"Successfully deleted memory: {content_hash}")]
            else:
...
Memories Archived: {report.memories_archived}"""
    ...
    # ========== PHASE 2-4: Memory Supercharge Handler Methods ==========

    async def handle_hybrid_retrieve_memory(self, arguments: dict) -> List[types.TextContent]:
        """Handle hybrid_retrieve_memory MCP tool call."""
        await self._ensure_storage_initialized()
        ...
        try:
            results = await storage.hybrid_retrieve(
                query=query,
                n_results=n_results,
            ...
            if not results:
                return [types.TextContent(type="text", text="No memories found matching the query.")]

            # Format output
            output_lines = [f"Found {len(results)} memories (hybrid search):"]
            output_lines.append("")

            for i, r in enumerate(results, 1):
                output_lines.append(f"**Memory {i}:**")
                output_lines.append(f"  Score: {r.get('rrf_score', 0):.3f}")
                sources = r.get('sources', [])
                if sources:
                    source_strs = [f"{s.get('method', 'unknown')}:#{s.get('rank', '?')}" for s in sources]
                    output_lines.append(f"  Sources: {', '.join(source_strs)}")
                content = r.get('content', '')[:300]
                output_lines.append(f"  Content: {content}{'...' if len(r.get('content', '')) > 300 else ''}")
                output_lines.append(f"  Tags: {r.get('tags', 'none')}")
                output_lines.append("")

            return [types.TextContent(type="text", text="\n".join(output_lines))]

        except Exception as e:
            logger.error(f"Error in hybrid_retrieve_memory: {str(e)}")
            return [types.TextContent(type="text", text=f"Error: {str(e)}")]

    async def handle_smart_retrieve_memory(self, arguments: dict) -> List[types.TextContent]:
        """Handle smart_retrieve_memory MCP tool call."""
        await self._ensure_storage_initialized()
        storage = self.storage
        ...
        try:
            result = await storage.smart_retrieve(
                query=query,
                n_results=n_results,
                use_hybrid=True
            )
            ...
            output_lines = [
                "**Query Understanding:**",
                f"  Original: {qu.get('original', query)}",
                f"  Cleaned: {qu.get('cleaned', query)}",
                f"  Filters: {qu.get('filters', {})}",
                f"  Time Filter: {qu.get('time_filter', 'none')}",
                f"  Expansions: {qu.get('expansions', [])}",
                f"  Intent: {qu.get('intent', 'general_search')}",
                ""
            ]

            memories = result.get("results", [])
            output_lines.append(f"Found {len(memories)} memories:")
            output_lines.append("")
            ...
            logger.error(f"Error in smart_retrieve_memory: {str(e)}")
            return [types.TextContent(type="text", text=f"Error: {str(e)}")]

    async def handle_find_duplicate_memories(self, arguments: dict) -> List[types.TextContent]:
        """Handle find_duplicate_memories MCP tool call."""
        await self._ensure_storage_initialized()
        storage = self.storage

        similarity_threshold = arguments.get("similarity_threshold", 0.90)
        max_results = arguments.get("max_results", 100)
        batch_size = arguments.get("batch_size", 100)

        try:
            duplicates = await storage.find_duplicates(
            ...
            if not duplicates:
                return [types.TextContent(type="text", text="No duplicate memories found above the similarity threshold.")]
            ...
            for i, dup in enumerate(duplicates, 1):
                output_lines.append(f"**Duplicate Pair {i}:**")
                output_lines.append(f"  Original: {dup.get('original_hash', '')[:16]}...")
                output_lines.append(f"  Duplicate: {dup.get('duplicate_hash', '')[:16]}...")
                output_lines.append(f"  Similarity: {dup.get('similarity', 0):.1%}")
                output_lines.append(f"  Recommendation: {dup.get('recommendation', 'review')}")

                original_preview = dup.get('original_content', '')[:100]
                output_lines.append(f"  Original preview: {original_preview}...")

                duplicate_preview = dup.get('duplicate_content', '')[:100]
                output_lines.append(f"  Duplicate preview: {duplicate_preview}...")
                output_lines.append("")

            return [types.TextContent(type="text", text="\n".join(output_lines))]

        except Exception as e:
            logger.error(f"Error in find_duplicate_memories: {str(e)}")
            return [types.TextContent(type="text", text=f"Error: {str(e)}")]

    async def handle_consolidate_memories(self, arguments: dict) -> List[types.TextContent]:
        """Handle consolidate_memories MCP tool call."""
        await self._ensure_storage_initialized()
        storage = self.storage

        hashes = arguments.get("hashes", [])
        keep_hash = arguments.get("keep_hash", "")
        merge_tags = arguments.get("merge_tags", True)

        if not hashes or not keep_hash:
            return [types.TextContent(type="text", text="Error: hashes and keep_hash are required")]

        if keep_hash not in hashes:
            return [types.TextContent(type="text", text="Error: keep_hash must be one of the hashes")]

        try:
            success, message = await storage.consolidate_memories(
                hashes=hashes,
                keep_hash=keep_hash,
            ...
            if success:
                output = f"✅ Successfully consolidated {len(hashes)} memories into {keep_hash[:16]}...\n{message}"
            else:
                output = f"❌ Consolidation failed: {message}"

            return [types.TextContent(type="text", text=output)]

        except Exception as e:
            logger.error(f"Error in consolidate_memories: {str(e)}")
            return [types.TextContent(type="text", text=f"Error: {str(e)}")]

    async def handle_cleanup_duplicates_auto(self, arguments: dict) -> List[types.TextContent]:
        """Handle cleanup_duplicates_auto MCP tool call."""
        await self._ensure_storage_initialized()
        storage = self.storage

        max_age_days = arguments.get("max_age_days", 90)
        similarity_threshold = arguments.get("similarity_threshold", 0.98)
        dry_run = arguments.get("dry_run", True)

        try:
            result = await storage.cleanup_old_duplicates(
            ...
            output_lines = [
                f"**Automatic Duplicate Cleanup ({mode})**",
                f"Max age: {max_age_days} days",
                f"Similarity threshold: {similarity_threshold:.0%}",
                "",
                f"Memories analyzed: {result.get('analyzed', 0)}",
                f"Duplicates found: {result.get('duplicates_found', 0)}",
                f"Duplicates {'would be ' if dry_run else ''}deleted: {result.get('deleted', 0)}",
            ]

            if result.get('errors'):
                output_lines.append(f"Errors: {len(result.get('errors', []))}")
                for err in result.get('errors', [])[:5]:
                    output_lines.append(f"  - {err}")

            if dry_run and result.get('duplicates_found', 0) > 0:
                output_lines.append("")
                output_lines.append("To execute cleanup, set dry_run=false")

            return [types.TextContent(type="text", text="\n".join(output_lines))]

        except Exception as e:
            logger.error(f"Error in cleanup_duplicates_auto: {str(e)}")
            return [types.TextContent(type="text", text=f"Error: {str(e)}")]

    # ========== WORKFLOW IMPROVEMENT HANDLERS ==========

    async def handle_find_stale_tasks(self, arguments: dict) -> List[types.TextContent]:
        """Handle find_stale_tasks MCP tool call."""
        await self._ensure_storage_initialized()
        storage = self.storage
        ...
        try:
            # Check if storage has the method (Cloudflare backend)
            if not hasattr(storage, 'find_stale_tasks'):
                return [types.TextContent(type="text", text="Error: find_stale_tasks is only available with Cloudflare backend")]

            result = await storage.find_stale_tasks(days=days, project=project)

            if not result:
                filter_text = f" for project '{project}'" if project else ""
                return [types.TextContent(type="text", text=f"No stale tasks found{filter_text} (in-progress > {days} days)")]

            output_lines = [
                f"**Stale Tasks (in-progress > {days} days)**",
                f"Project filter: {project or 'all'}",
                f"Found: {len(result)} stale tasks",
                ""
            ]

            for i, task in enumerate(result, 1):
                output_lines.append(f"**{i}. {task.get('content', 'No content')[:100]}...**")
                output_lines.append(f"   Hash: {task.get('content_hash', 'N/A')[:16]}...")
                output_lines.append(f"   Created: {task.get('created_at_iso', 'N/A')}")
                output_lines.append(f"   Days stale: {task.get('days_stale', 'N/A')}")
                output_lines.append("")

            return [types.TextContent(type="text", text="\n".join(output_lines))]

        except Exception as e:
            logger.error(f"Error in find_stale_tasks: {str(e)}")
            return [types.TextContent(type="text", text=f"Error: {str(e)}")]

    async def handle_load_project_context(self, arguments: dict) -> List[types.TextContent]:
        """Handle load_project_context MCP tool call."""
        await self._ensure_storage_initialized()
        storage = self.storage

        project = arguments.get("project")
        if not project:
            return [types.TextContent(type="text", text="Error: project is required")]
        ...
        try:
            # Check if storage has the method (Cloudflare backend)
            if not hasattr(storage, 'load_project_context'):
                return [types.TextContent(type="text", text="Error: load_project_context is only available with Cloudflare backend")]

            result = await storage.load_project_context(
                project=project,
            ...
            if pending:
                output_lines.append(f"## Pending Tasks ({len(pending)})")
                for task in pending[:5]:
                    output_lines.append(f"- {task.get('content', '')[:100]}...")
                if len(pending) > 5:
                    output_lines.append(f"  _...and {len(pending) - 5} more_")
                output_lines.append("")

            # In-progress tasks
            in_progress = result.get("in_progress_tasks", [])
            if in_progress:
                output_lines.append(f"## In-Progress Tasks ({len(in_progress)})")
                for task in in_progress[:5]:
                    output_lines.append(f"- {task.get('content', '')[:100]}...")
                if len(in_progress) > 5:
                    output_lines.append(f"  _...and {len(in_progress) - 5} more_")
                output_lines.append("")

            # Recent decisions
            decisions = result.get("recent_decisions", [])
            if decisions:
                output_lines.append(f"## Recent Decisions ({len(decisions)})")
                for d in decisions[:3]:
                    output_lines.append(f"- {d.get('content', '')[:100]}...")
                output_lines.append("")

            # Recent errors
            errors = result.get("recent_errors", [])
            if errors:
                output_lines.append(f"## Recent Errors ({len(errors)})")
                for e in errors[:3]:
                    output_lines.append(f"- {e.get('content', '')[:100]}...")
                output_lines.append("")

            # Client chats
            chats = result.get("client_chats", [])
            if chats:
                output_lines.append(f"## Client Chats ({len(chats)})")
                for c in chats[:3]:
                    output_lines.append(f"- {c.get('content', '')[:100]}...")
                output_lines.append("")

            return [types.TextContent(type="text", text="\n".join(output_lines))]

        except Exception as e:
            logger.error(f"Error in load_project_context: {str(e)}")
            return [types.TextContent(type="text", text=f"Error: {str(e)}")]

    async def handle_save_session_state(self, arguments: dict) -> List[types.TextContent]:
        """Handle save_session_state MCP tool call."""
        await self._ensure_storage_initialized()
        storage = self.storage
        ...
        if not all([project, current_task, progress, next_steps]):
            return [types.TextContent(type="text", text="Error: project, current_task, progress, and next_steps are required")]

        try:
            # Check if storage has the method (Cloudflare backend)
            if not hasattr(storage, 'save_session_state'):
                return [types.TextContent(type="text", text="Error: save_session_state is only available with Cloudflare backend")]

            success, message = await storage.save_session_state(
                project=project,
            ...
            if success:
                return [types.TextContent(type="text", text=f"✅ Session state saved for project '{project}'\n{message}")]
            else:
                return [types.TextContent(type="text", text=f"❌ Failed to save session state: {message}")]

        except Exception as e:
            logger.error(f"Error in save_session_state: {str(e)}")
            return [types.TextContent(type="text", text=f"Error: {str(e)}")]

    async def handle_get_interrupted_sessions(self, arguments: dict) -> List[types.TextContent]:
        """Handle get_interrupted_sessions MCP tool call."""
        await self._ensure_storage_initialized()
        storage = self.storage
        ...
        try:
            ...
            if not hasattr(storage, 'get_interrupted_sessions'):
                return [types.TextContent(type="text", text="Error: get_interrupted_sessions is only available with Cloudflare backend")]

            result = await storage.get_interrupted_sessions(project=project, days=days)

            if not result:
                filter_text = f" for project '{project}'" if project else ""
                return [types.TextContent(type="text", text=f"No interrupted sessions found{filter_text} in last {days} days")]

            output_lines = [
                f"**Interrupted Sessions (last {days} days)**",
                f"Project filter: {project or 'all'}",
                f"Found: {len(result)} sessions",
                ""
            ]

            for i, session in enumerate(result, 1):
                output_lines.append(f"**{i}. Session from {session.get('created_at_iso', 'N/A')}**")
                ...
                output_lines.append(f"   Hash: {session.get('content_hash', 'N/A')[:16]}...")
                output_lines.append("")

            return [types.TextContent(type="text", text="\n".join(output_lines))]

        except Exception as e:
            logger.error(f"Error in get_interrupted_sessions: {str(e)}")
            return [types.TextContent(type="text", text=f"Error: {str(e)}")]

    async def handle_get_memory_analytics(self, arguments: dict) -> List[types.TextContent]:
        """Handle get_memory_analytics MCP tool call."""
        await self._ensure_storage_initialized()
        storage = self.storage
        ...
        try:
            # Check if storage has the method (Cloudflare backend)
            if not hasattr(storage, 'get_memory_analytics'):
                return [types.TextContent(type="text", text="Error: get_memory_analytics is only available with Cloudflare backend")]

            result = await storage.get_memory_analytics(project=project, days=days)
            ...
            output_lines = [
                ...
                "",
                f"**Total Memories:** {result.get('total_count', 0)}",
                ""
            ]
            ...
            if by_project:
                output_lines.append("## By Project")
                for proj, count in sorted(by_project.items(), key=lambda x: x[1], reverse=True):
                    output_lines.append(f"- {proj}: {count}")
                output_lines.append("")

            # By status
            by_status = result.get("by_status", {})
            if by_status:
                output_lines.append("## By Status")
                for status, count in sorted(by_status.items(), key=lambda x: x[1], reverse=True):
                    output_lines.append(f"- {status}: {count}")
                output_lines.append("")

            # Time distribution
            by_time = result.get("by_time", {})
            if by_time:
                output_lines.append("## Time Distribution")
                for period, count in by_time.items():
                    output_lines.append(f"- {period}: {count}")
                output_lines.append("")

            return [types.TextContent(type="text", text="\n".join(output_lines))]

        except Exception as e:
            logger.error(f"Error in get_memory_analytics: {str(e)}")
            return [types.TextContent(type="text", text=f"Error: {str(e)}")]

    async def handle_cleanup_old_memories(self, arguments: dict) -> List[types.TextContent]:
        """Handle cleanup_old_memories MCP tool call."""
        await self._ensure_storage_initialized()
        storage = self.storage
        ...
        try:
            # Check if storage has the method (Cloudflare backend)
            if not hasattr(storage, 'cleanup_old_memories'):
                return [types.TextContent(type="text", text="Error: cleanup_old_memories is only available with Cloudflare backend")]

            result = await storage.cleanup_old_memories(
            ...
            output_lines = [
                f"# Memory Cleanup - {mode}",
                f"Older than: {older_than_days} days",
                f"Types filter: {memory_types or 'all'}",
                f"Exclude tags: {exclude_tags or 'none'}",
                "",
                f"**Found:** {result.get('found', 0)} memories",
                f"**{'Would delete' if dry_run else 'Deleted'}:** {result.get('deleted', 0)}",
                f"**Skipped (protected):** {result.get('skipped', 0)}",
            ]
            ...
            if dry_run and result.get('found', 0) > 0:
                output_lines.append("")
                output_lines.append("⚠️ This was a DRY RUN - no data was deleted.")
                output_lines.append("To actually delete, set dry_run=false")

            # Show sample of what would be deleted
            samples = result.get('samples', [])
            if samples:
                output_lines.append("")
                output_lines.append("**Sample of affected memories:**")
                for sample in samples[:5]:
                    output_lines.append(f"- {sample.get('content', '')[:80]}...")
                    output_lines.append(f"  Type: {sample.get('memory_type', 'N/A')}, Created: {sample.get('created_at_iso', 'N/A')}")

            return [types.TextContent(type="text", text="\n".join(output_lines))]

        except Exception as e:
            logger.error(f"Error in cleanup_old_memories: {str(e)}")
            return [types.TextContent(type="text", text=f"Error: {str(e)}")]
